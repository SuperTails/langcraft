...
pub fn tokenize(s: &str) -> Result<Vec<Token>, String> {
    ...
    while let Some(c) = iter.next() {
        match (c, iter.peek()) {
            ('+', Some('=')) => { add_binary_op(&mut result, &mut current, BinaryOp::AddAssign); iter.next(); },
            ('-', Some('=')) => { add_binary_op(&mut result, &mut current, BinaryOp::SubAssign); iter.next(); },
            ('*', Some('=')) => { add_binary_op(&mut result, &mut current, BinaryOp::MulAssign); iter.next(); },
            ('/', Some('=')) => { add_binary_op(&mut result, &mut current, BinaryOp::DivAssign); iter.next(); },
            ('%', Some('%')) => { add_binary_op(&mut result, &mut current, BinaryOp::ModAssign); iter.next(); },
            ('>', Some('<')) => { add_binary_op(&mut result, &mut current, BinaryOp::Swap); iter.next(); },
            ('>', Some('=')) => { flush(&mut result, &mut current); result.push(Token::GreaterThanEq); iter.next(); },
            ('<', Some('=')) => { flush(&mut result, &mut current); result.push(Token::LessThanEq); iter.next(); },
            ('&', Some('&')) => { flush(&mut result, &mut current); result.push(Token::And); iter.next(); },
            ...
            (';', _) => { flush(&mut result, &mut current); result.push(Token::Semicolon) },
            ('{', _) => { flush(&mut result, &mut current); result.push(Token::OpenCurly) },
            ('}', _) => { flush(&mut result, &mut current); result.push(Token::CloseCurly) },
            ('(', _) => { flush(&mut result, &mut current); result.push(Token::OpenParen) },
            (')', _) => { flush(&mut result, &mut current); result.push(Token::CloseParen) },
...
pub fn parse(s: &str) -> Result<Unit, String> {
    ...
    match parser.parse_unit() {
        Ok(result) => Ok(result),
        Err(err) => {
            Err(err + &format!(" parser state: {:?}", parser.tokens))
        }
    }
}
...
impl Parser<'_> {
    ...
    pub fn next_token_result(&mut self) -> Result<&Token, String> {
        self.next_token().ok_or_else(|| "unexpected end of tokens".to_string())
    }
    ...
    pub fn parse_conditions(&mut self) -> Result<Vec<Condition>, String> {
        ...
        loop {
            ...
            let relation = Relation::try_from(self.next_token_result()?.clone()).map_err(|_| "expected relation")?;
            let rhs = self.parse_expr()?;

            conds.push(Condition {
                inverted: false,
                lhs,
                kind: ConditionKind::Relation {
                    relation,
                    rhs,
                }
            });
