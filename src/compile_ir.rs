...
pub fn compile_module(module: &Module, options: &BuildOptions) -> Vec<McFunction> {
    ...
    let mut funcs = funcs
        .into_iter()
        .map(|block| {
            reify_block(block, &clobber_list, &func_starts, &globals)
        })
        .collect::<Vec<_>>();
...
#[allow(clippy::reversed_empty_ranges)]
fn reify_block(AbstractBlock { needs_prolog, mut body, term, parent }: AbstractBlock, clobber_list: &HashMap<String, BTreeSet<ScoreHolder>>, func_starts: &HashMap<String, McFuncId>, globals: &GlobalVarList) -> McFunction {
...
fn compile_block_end(block_end: &BlockEnd, body_cmds: usize, parent: &Function, clobbers: BTreeSet<ScoreHolder>, func_starts: &HashMap<String, McFuncId>, globals: &GlobalVarList) -> Vec<Command> {
    let mut cmds = Vec::new();
    ...
    let dests: Either<Vec<(BlockEdge, McFuncId)>, McFuncId> = match block_end {
        ...
        BlockEnd::Normal(t) => {
            let (tmp, dests) = compile_terminator(&parent, &t, clobbers, globals);
            cmds.extend(tmp);
            dests
        }
    };
